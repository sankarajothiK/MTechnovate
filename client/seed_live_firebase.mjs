...
async function seedLiveFirestore() {
  ...
  try {
    ...
    for (const tmpl of seedData.emailTemplates) {
      await setDoc(doc(db, 'emailTemplates', String(tmpl.template_type || tmpl.id)), {
        ...tmpl,
        updatedAt: serverTimestamp()
      }, { merge: true });
    }
    ...
    console.log('Firestore Live Seeding Note:', err.message);
    if (err.code === 'permission-denied') {
      console.log('Note: If Firestore security rules are in strict mode on the cloud, publish firestore.rules via Firebase Console.');
    }
    process.exit(0);
  }
}
