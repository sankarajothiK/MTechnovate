...
export default function AdminApplications() {
  ...
  const handleOpenResumePreview = async (app) => {
    ...
    try {
      const rawUrl = await api.getResumeUrl(app);
      const blobUrl = api.createResumeBlobUrl(rawUrl);
      setPreviewResumeUrl(blobUrl);
    } catch (e) {
      console.warn('Error loading resume preview:', e);
      setPreviewResumeUrl(app.resume_url || app.resumeUrl || '');
    } finally {
  ...
  const handleCloseResumePreview = () => {
    if (previewResumeUrl && previewResumeUrl.startsWith('blob:')) {
      try {
        URL.revokeObjectURL(previewResumeUrl);
      } catch {}
    }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {/* MODAL 4: Interactive In-App Resume Preview */}
      {previewResumeApp && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-900/70 dark:bg-dark-950/85 backdrop-blur-md">
          <div className="relative w-full max-w-4xl h-[88vh] bg-white dark:bg-dark-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
            {/* Modal Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-dark-950">
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-xl bg-cyan-500/10 dark:bg-cyan-500/20 text-cyan-600 dark:text-cyan-400 border border-cyan-500/20">
                  <FileText className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                    <span>{previewResumeApp.full_name || previewResumeApp.candidateName}'s Resume</span>
                    <span className="text-xs px-2 py-0.5 rounded-md bg-slate-200 dark:bg-dark-800 text-slate-700 dark:text-slate-300 font-mono font-normal">
                      {previewResumeApp.job_title || 'Application'}
                    </span>
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-mono mt-0.5">
                    {previewResumeApp.resume_filename || 'Candidate_Resume.pdf'} • Applied {previewResumeApp.applied_at ? previewResumeApp.applied_at.substring(0, 10) : 'Recent'}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                {previewResumeUrl && (
                  <>
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-dark-800 dark:hover:bg-dark-700 text-slate-700 dark:text-slate-200 text-xs font-semibold transition-colors"
                    ...
                    <a
                      href={previewResumeUrl}
                      download={previewResumeApp.resume_filename || `${(previewResumeApp.full_name || 'candidate').replace(/\s+/g, '_')}_Resume.pdf`}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-semibold shadow-sm transition-colors"
                ...
                <button
                  ...
                  className="p-2 rounded-xl text-slate-400 hover:text-slate-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-dark-800 transition-colors cursor-pointer"
            ...
            {/* Modal Body / Viewer */}
            <div className="flex-1 bg-slate-100 dark:bg-dark-950 p-2 sm:p-4 overflow-hidden flex items-center justify-center">
              {previewLoading ? (
                <div className="flex flex-col items-center gap-3 text-slate-500 font-mono text-xs">
                  <Loader2 className="w-8 h-8 animate-spin text-cyan-500" />
                  <span>Loading candidate resume document...</span>
                </div>
              ) : previewResumeUrl ? (
                <iframe
                  src={previewResumeUrl}
                  title="Candidate Resume Preview"
                  className="w-full h-full rounded-xl border border-slate-200 dark:border-slate-800 bg-white shadow-inner"
                />
              ) : (
                <div className="text-center space-y-2 text-slate-500">
                  <AlertCircle className="w-8 h-8 mx-auto text-amber-500" />
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Resume File Not Available</p>
                  <p className="text-xs">No resume file was attached with this candidate's submission.</p>
                </div>
              )}
            </div>
          </div>
